...
Color4f Raytracer::applyShader(const int x, const int y, const float t = 0.0f) {
	...
	/*if (x >= 223 -5 && x <= 223 + 5 && y >= 13 -5 && y <= 13+5) {
...
Color4f Raytracer::applyShaderInternal(RTCRayHitWithIor rtcRayHitWithIor, float t, int depth, float prudence)
{
	...
	if (depth > 4)  return Color4f{ 0,0,0, 1 };// sphericalMap->getTexel(toIntersectionVector);
	//if (!russianRouleteBasedOnPrudence(prudence)) return resultColor;
...
float Raytracer::getGeometryTerm(Vector3 omegaI, IntersectionInfo intersectionInfo, RTCIntersectContext context) {
	...
	if (visibilityTerm == 1)
	{
		...
		if (sourceNormal.DotProduct(-intersectionInfo.vectorToLight) < 0) {
			sourceNormal *= -1;
		}
...
Color4f Raytracer::applyGlassShader(RTCRayHitWithIor rtcRayHitWithIor, IntersectionInfo intersectionInfo, float t, int depth, float prudence) {
	...
	/*if (resultColor.r != 0 || resultColor.g != 0 || resultColor.b != 0) {
		printf("sa");
	}*/
	return resultColor;
}
