...
Color4f Raytracer::applyShaderInternal(RTCRayHitWithIor rtcRayHitWithIor, float t, int depth, float prudence)
{
	...
	if (depth > 4)  return Color4f{ 0,0,0,1 };//return sphericalMap->getTexel(toIntersectionVector);
	if (!russianRouleteBasedOnPrudence(prudence)) return Color4f{0,0,0,1};
	...
	if (rtcRayHitWithIor.rtcRayHit.hit.geomID != RTC_INVALID_GEOMETRY_ID)
	{
		...
		/*Color4f emmision = Color4f{ intersectionInfo.material->emission.x, intersectionInfo.material->emission.y, intersectionInfo.material->emission.z };
		...
		case PHONG_SHADER:
			...
			break;
		case GLASS_SHADER:
			resultColor = applyGlassShader(rtcRayHitWithIor, intersectionInfo, t, depth, prudence);
			break;
		case WHITTED_SHADER:
			resultColor = applyWhittedShader(rtcRayHitWithIor, intersectionInfo, t, depth, prudence);
			break;
		case NORMAL_SHADER:
			resultColor = applyNormalShader(rtcRayHitWithIor, intersectionInfo, t);
			break;
		case  PHYSICALLY_BASED_SHADER:
			resultColor = applyPhysicallyBasedShader(rtcRayHitWithIor, intersectionInfo, t, depth, context, prudence);
			break;
		}
	}
	else {
...
bool Raytracer::russianRouleteBasedOnPrudence(float prudence) {
	float random = Random();
	return random < prudence;  
}
...
Vector3 Raytracer::sampleHemisphere(Vector3 normal, float& pdf) {
	...
	Matrix3x3 transformationMatrix = Matrix3x3(O1, O2, normal);
	
	Vector3 omegaI = Vector3{ x, y, z };

	omegaI = transformationMatrix * omegaI;

	pdf = normal.DotProduct(omegaI) / float(M_PI);
...
Color4f Raytracer::applyPhysicallyBasedShader(RTCRayHitWithIor rtcRayHitWithIor, IntersectionInfo intersectionInfo, float t, int depth, RTCIntersectContext context, float prudence) {
	depth++;

	Color4f resultColor = Color4f{0,0,0,1};
	Color4f emission = Color4f{ intersectionInfo.material->emission.x, intersectionInfo.material->emission.y, intersectionInfo.material->emission.z, 1 };
	...
	prudence = prudence * max(fR.r, max(fR.g, fR.b));
	...
	Color4f areaForm = fR * sourceColor * getGeometryTerm(omegaI, intersectionInfo, context);
...
float Raytracer::getGeometryTerm(Vector3 omegaI, IntersectionInfo intersectionInfo, RTCIntersectContext context) {
	float randomX = Random(-50, 50); //generate in interval -50 and 50
	float randomY = Random(-50, 50); //generate in interval -50 and 50
	Vector3 sourcePoint(randomX, randomY, 489); //489 heaight of source

	float geometryTerm = 0.0f;
	...
	float visibilityTerm = castShadowRay(intersectionInfo, context);

	if (visibilityTerm == 1)
	{
		Vector3 sourceNormal{ 0, 0, -1 };
		geometryTerm = ((intersectionInfo.normal.DotProduct(intersectionInfo.vectorToLight) * sourceNormal.DotProduct(-intersectionInfo.vectorToLight))
			/ sqr(intersectionInfo.dstToLight)) * 10000;
	}
...
Color4f Raytracer::applyGlassShader(RTCRayHitWithIor rtcRayHitWithIor, IntersectionInfo intersectionInfo, float t, int depth, float prudence) {
	...
	if (sqrCos2 > 0) {
		...
		reflectedRayHitWithIor = createRayWithEmptyHitAndIor(intersectionInfo.intersectionPoint, dirOfReflectedRay, FLT_MAX, 0.01f, ior2);

		transmittedRayHitWithIor = createRayWithEmptyHitAndIor(intersectionInfo.intersectionPoint, dirOfTransmittedRay, FLT_MAX, 0.01f, ior2);
		prudence *= 0.99;
		resultColor = resultColor + applyShaderInternal(transmittedRayHitWithIor, t, depth, prudence) * T;
		resultColor = resultColor + applyShaderInternal(reflectedRayHitWithIor, t, depth, prudence) * R;
	}
	else {
		reflectedRayHitWithIor = createRayWithEmptyHitAndIor(intersectionInfo.intersectionPoint, dirOfReflectedRay, FLT_MAX, 0.01f, ior2);
		resultColor = resultColor + applyShaderInternal(reflectedRayHitWithIor, t, depth,prudence);
	}
...
Color4f Raytracer::applyWhittedShader(RTCRayHitWithIor rtcRayHitWithIor, IntersectionInfo intersectionInfo, float t, int depth, float prudence) {
	float normalViewScalarProduct = intersectionInfo.normal.DotProduct(-intersectionInfo.viewToIntersectionVector);
	Vector3 lr = 2 * (normalViewScalarProduct)* intersectionInfo.normal - (-intersectionInfo.viewToIntersectionVector);

	RTCRayHitWithIor reflectedRay = createRayWithEmptyHitAndIor(intersectionInfo.intersectionPoint, lr, FLT_MAX, 0.01f, rtcRayHitWithIor.ior);

	return applyShaderInternal(reflectedRay, t, ++depth,  prudence);// * Color4f{ intersectionInfo.material->getDiffuse(intersectionInfo.tex_coord).x, intersectionInfo.material->getDiffuse(intersectionInfo.tex_coord).y, intersectionInfo.material->getDiffuse(intersectionInfo.tex_coord).z, 1 };
}
...
float Raytracer::castShadowRay(IntersectionInfo intersectionInfo, RTCIntersectContext context) {
	RTCRay rayFromIntersectPointToLight = createRay(intersectionInfo.intersectionPoint, intersectionInfo.vectorToLight, intersectionInfo.dstToLight, 0.01f);
